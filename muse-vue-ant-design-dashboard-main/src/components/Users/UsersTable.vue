<template>
	<a-card :bordered="false" class="header-solid h-full" :bodyStyle="{ padding: 0 }">
		<template #title>
			<!-- TÃ­tulo arriba, controles abajo alineados -->
		...
		>
				...
				>
					...
					<div class="avatar-info">
						<h6 class="table-firstcol__title">
							<span class="table-firstcol__caret" aria-hidden="true">{{ isExpanded(record) ? 'â–¾' : 'â–¸' }}</span>
			...
			<template slot="actions" slot-scope="_, row">
				<!-- Si estÃ¡ eliminado, queda como registro (solo lectura) -->
				...
				<div v-else class="table-actions">
					...
					<a-popconfirm
						title="Â¿EstÃ¡s seguro de resetear la contraseÃ±a de este usuario? Se generarÃ¡ una contraseÃ±a temporal y se le notificarÃ¡."
						ok-text="SÃ­"
						cancel-text="No"
			...
			<template slot="expandedRowRender" slot-scope="record">
				<div class="user-expanded">
					<div class="user-expanded__header">
						...
						<div class="user-expanded__actions">
							...
							<template v-else-if="record && record._raw && record._raw.isInactive">
								<a-popconfirm
									title="Â¿Deseas volver a activar este usuario?"
									ok-text="SÃ­, activar"
									cancel-text="No"
					...
					<div class="user-expanded__grid">
						...
						<div class="user-expanded__item">
							<div class="user-expanded__label">FinalizaciÃ³n</div>
...
export default ({
	...
	methods: {
		...
		canManagePermissions(row) {
			// Obtener el usuario actual logueado de mÃºltiples fuentes
			...
			console.log('canManagePermissions - currentUserId:', currentUserId);
			console.log('canManagePermissions - currentUser:', currentUser);
			...
			console.log('canManagePermissions - targetUserId:', targetUserId);
			console.log('canManagePermissions - targetUserRole:', targetUserRole);
			console.log('canManagePermissions - row:', row);
			
			// Roles que pueden gestionar permisos
			const allowedRoles = ['Super usuario'];
			
			// Verificar si el usuario actual tiene un rol permitido
			const canManage = allowedRoles.includes(currentRole);
			console.log('canManagePermissions - allowedRoles:', allowedRoles);
			console.log('canManagePermissions - canManage:', canManage);
			...
			console.log('canManagePermissions - isTargetSuperUser:', isTargetSuperUser);
			console.log('canManagePermissions - isCurrentSuperUser:', isCurrentSuperUser);
			
			// No permitir gestionar permisos de otros Super usuarios (inmutables)
			if (isTargetSuperUser && currentUserId !== targetUserId) {
		...
		getPermissionsDisabledMessage(row) {
			...
			// Mensajes especÃ­ficos para cada restricciÃ³n
			if (!hasAllowedRole) {
				return 'No tienes permisos para gestionar permisos de usuarios';
			}
			
			if (isTargetSuperUser && currentUserId !== targetUserId) {
				return 'No puedes gestionar permisos de otros Super usuarios';
			}
		...
		canEditUser(row) {
			...
			// Verificar si el usuario estÃ¡ inactivo o eliminado
			const isInactive = !!(row && row._raw && row._raw.isInactive);
			const isDeleted = !!(row && row._raw && row._raw.isDeleted);
			
			// Si estÃ¡ inactivo o eliminado, no se puede editar
			...
			// No permitir editar Super usuarios (excepto si el actual tambiÃ©n es Super usuario)
			const isTargetSuperUser = targetUserRole.toLowerCase() === 'super usuario';
			const isCurrentSuperUser = currentRole.toLowerCase() === 'super usuario';
			
			// No permitir editar a otros Super usuarios (inmutables)
			if (isTargetSuperUser && currentUserId !== targetUserId) {
				return false;
			}
			
			// Los usuarios no pueden editarse a sÃ­ mismos (excepto Super usuarios y Administradores)
		...
		getEditDisabledMessage(row) {
			...
			// Mensajes especÃ­ficos para cada restricciÃ³n
			if (isDeleted) {
				return 'Este usuario estÃ¡ eliminado y no se puede editar';
			}
			
			if (isInactive) {
				return 'Este usuario estÃ¡ desactivado y no se puede editar';
			}
			...
			if (isTargetSuperUser && currentUserId !== targetUserId) {
				return 'No puedes editar a otros Super usuarios';
			}
		...
		canResetPassword(row) {
			...
			// Si estÃ¡ inactivo o eliminado, no se puede resetear contraseÃ±a
			if (isInactive || isDeleted) {
				return false;
			}
			
			// Roles que pueden resetear contraseÃ±as
			const allowedRoles = ['Super usuario'];
			
			// Verificar si el usuario actual tiene un rol permitido
			const canReset = allowedRoles.includes(currentRole);
			
			// Si no tiene rol permitido, no puede resetear contraseÃ±as de nadie
			if (!canReset) {
				return false;
			}
			
			// No permitir resetear contraseÃ±a de Super usuarios (excepto si el actual tambiÃ©n es Super usuario)
			const isTargetSuperUser = targetUserRole.toLowerCase() === 'super usuario';
			const isCurrentSuperUser = currentRole.toLowerCase() === 'super usuario';
			
			// No permitir resetear contraseÃ±a de otros Super usuarios (inmutables)
			if (isTargetSuperUser && currentUserId !== targetUserId) {
				return false;
			}
			
			// Los usuarios no pueden resetear su propia contraseÃ±a (excepto Super usuarios)
			if (currentUserId === targetUserId && !isCurrentSuperUser) {
				return false;
			}
		...
		getResetPasswordDisabledMessage(row) {
			...
			// Mensajes especÃ­ficos para cada restricciÃ³n
			if (isDeleted) {
				return 'Este usuario estÃ¡ eliminado y no se puede resetear la contraseÃ±a';
			}
			
			if (isInactive) {
				return 'Este usuario estÃ¡ desactivado y no se puede resetear la contraseÃ±a';
			}
			...
			if (!hasAllowedRole) {
				return 'No tienes permisos para resetear contraseÃ±as';
			}
			
			if (isTargetSuperUser && currentUserId !== targetUserId) {
				return 'No puedes resetear la contraseÃ±a de otros Super usuarios';
			}
			
			if (currentUserId === targetUserId && !isCurrentSuperUser) {
				return 'No puedes resetear tu propia contraseÃ±a';
			}
...
/* asegurar que wrappers de Popconfirm no rompan alineaciÃ³n */
.user-expanded__actions > * {
	display: inline-flex;
}
